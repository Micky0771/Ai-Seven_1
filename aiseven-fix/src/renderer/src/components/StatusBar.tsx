  ...
  return (
    <div className="h-6 bg-slate-950 border-t border-slate-800 flex items-center px-4 gap-4 shrink-0">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs text-slate-500">Ollama {aiHealth?.ollama ? 'conectado' : 'desconectado'}</span>
      </div>
      {aiHealth?.ollama && aiHealth.models.length > 0 && (
        <><div className="w-px h-3 bg-slate-700"></div><span className="text-xs text-slate-600">{aiHealth.models[0]}</span></>
      )}
      <div className="flex-1"></div>
      <span className="text-xs text-slate-600">{subjects.length} ramo{subjects.length !== 1 ? 's' : ''} · {documents.length} doc{documents.length !== 1 ? 's' : ''}</span>
