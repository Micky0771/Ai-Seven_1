...
export default function ChatPanel() {
  ...
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [chatMessages, isChatLoading])
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setUseContext(!useContext)}
            className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border transition-colors ${useContext && hasDocuments ? 'bg-indigo-900/50 border-indigo-600 text-indigo-300' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
            <span>{useContext && hasDocuments ? '📚' : '💬'}</span>
            <span>{useContext && hasDocuments ? 'Con contexto' : 'Sin contexto'}</span>
          </button>
          {chatMessages.length > 0 && (
            <button onClick={clearChat} className="text-xs text-slate-500 hover:text-slate-300">🗑 Limpiar</button>
          )}
        </div>
      </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4 py-12 text-center">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                {hasDocuments ? `Tengo acceso a ${documents.length} documento(s). ¡Pregúntame!` : 'Agrega documentos para que pueda responder con tu material.'}
              </p>
            </div>
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              {(hasDocuments
                ? ['¿Cuál es el tema principal?', 'Hazme un resumen', '¿Qué debo estudiar?']
      ...
      <div className="p-4 border-t border-slate-800 shrink-0">
        <div className="flex gap-3 items-end">
          <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
            placeholder="Escribe tu pregunta... (Enter para enviar)" rows={1} disabled={isChatLoading}
          ...
          <button onClick={handleSend} disabled={!input.trim() || isChatLoading}
            className="w-10 h-10 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 rounded-xl flex items-center justify-center transition-colors shrink-0">
...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-sm ${isUser ? 'bg-slate-700' : 'bg-indigo-600'}`}>
        {isUser ? '👤' : '🤖'}
      </div>
      <div className={`max-w-[75%] flex flex-col gap-1 ${isUser ? 'items-end' : 'items-start'}`}>
        <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${isUser ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 rounded-tl-none'}`}>
          {message.content}
        </div>
