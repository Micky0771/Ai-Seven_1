  ...
  return (
    <div className="w-64 bg-slate-950 border-r border-slate-800 flex flex-col shrink-0">
      <div className="p-4 border-b border-slate-800">
        <div className="flex items-center justify-between mb-1">
          ...
          <button onClick={() => setShowForm(!showForm)}
            ...
            <span className="text-white text-sm font-bold">{showForm ? '×' : '+'}</span>
          </button>
        </div>

        {showForm && (
          <form onSubmit={handleCreate} className="mt-3 flex flex-col gap-2">
            <input autoFocus type="text" placeholder="Nombre del ramo" value={form.name}
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {subjects.length === 0 && (
          <div className="text-center text-slate-600 text-xs mt-8 px-4">No hay ramos aún.<br />Crea uno con el botón +</div>
        )}
...
function SubjectItem({ subject, isActive, onSelect, onDelete }: {
  subject: Subject; isActive: boolean; onSelect: () => void; onDelete: () => void
}) {
  const [hover, setHover] = useState(false)
  return (
    <div className={`group flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-colors mb-0.5 ${isActive ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}
